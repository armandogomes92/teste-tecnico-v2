...
public class VehicleCountByTollPlazaReportHandlerTests
{
    [Fact]
    public async Task Handle_Success_SetsStatusAndSavesReport()
    {
        ...
        var mockLogger = new Mock<ILogger<VehicleCountByTollPlazaReportHandler>>();
        var handler = new VehicleCountByTollPlazaReportHandler(mockCache.Object, mockLogger.Object);
        var message = new VehicleCountByTollPlazaReportMessage { TollPlazaId = 1, ReportDate = DateOnly.FromDateTime(DateTime.Now), ReportId = Guid.NewGuid() };
